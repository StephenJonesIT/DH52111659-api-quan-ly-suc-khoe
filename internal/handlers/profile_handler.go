...
func (h *ProfileHandler) CreateProfileHandler(ctx *gin.Context) {
	...
	var profileResponse models.ProfileResponse = models.ProfileResponse{
		...
		BMI:        utils.CalculateBMI(profile.Weight, profile.Height),
	}
	ctx.JSON(http.StatusCreated, common.NewResponseNormal("Profile created successfully", profileResponse))
}
...
func (h *ProfileHandler) UpdateProfileHandler(ctx *gin.Context) {
	...
	{
		if err != nil {
			ctx.JSON(http.StatusBadRequest, common.NewResponseError("Invalid profile ID"))
			return
		}
